...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["username"]) || empty($_POST["password"])) {
        ...
        $stmt = $conn->prepare("SELECT PupilsID FROM student_login WHERE Username=? AND Password=?");
        ...
        if ($stmt->num_rows == 1) {
            ...
            $stmt->bind_result($pupilID);
            $stmt->fetch();

            //  Student ID saved
            $_SESSION["logged_in_pupil"] = $pupilID;

            // Take you to Student.php
            ...
            <div class="card-body p-4 p-lg-5 text-black" style="width: 100%;">
              <form method="POST">
                <div class="d-flex align-items-start mb-3 pb-1">
                  <span class="h1 fw-bold mb-0" id="roleTitle">Sign into your student account</span>
                </div>
