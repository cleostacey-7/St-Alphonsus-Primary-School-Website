...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["username"]) || empty($_POST["password"])) {
        ...
        $stmt = $conn->prepare("SELECT TeachersID FROM Teacher_Login WHERE Username=? AND Password=?");
        ...
        if ($stmt->num_rows == 1) {
            ...
            $stmt->bind_result($TeachersID);
            $stmt->fetch();

            //  Teachers ID saved
            $_SESSION["logged_in_teacher"] = $TeachersID;

            // Take you to Teachers.php
            ...
            <div class="card-body p-4 p-lg-5 text-black" style="width: 100%;">
              <form method="POST">
                <div class="d-flex align-items-start mb-3 pb-1">
                  <span class="h1 fw-bold mb-0" id="roleTitle">Sign into your teacher account</span>
                </div>
